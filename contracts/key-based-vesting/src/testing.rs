...
fn claim_native() {
    ...
    let msg = ExecuteMsg::Claim {
        vesting_keys: vec![
            "test_claim_native_notfound".to_string(),
            "test_claim_native_valid".to_string(),
        ],
        recipient: None,
    };
    ...
    match res.unwrap_err() {
        StdError::GenericErr { msg, .. } => assert_eq!(
            msg,
            "vesting entry is not found the key \"test_claim_native_notfound\""
        ),
        _ => panic!("should not enter"),
    }
...
fn claim_cw20() {
    ...
    let msg = ExecuteMsg::Claim {
        vesting_keys: vec![
            "test_claim_cw20_notfound".to_string(),
            "test_claim_cw20_valid".to_string(),
        ],
        recipient: None,
    };
    ...
    match res.unwrap_err() {
        StdError::GenericErr { msg, .. } => assert_eq!(
            msg,
            "vesting entry is not found the key \"test_claim_cw20_notfound\""
        ),
        _ => panic!("should not enter"),
    }
    ...
    assert_eq!(
        ...
        vec![
            Attribute::new("action", "claim"),
            Attribute::new("address", "addr0001"),
            Attribute::new("vesting_key", "test_claim_cw20_valid"),
